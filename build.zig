...
pub fn build(b: *std.Build) void {
    ...
    const clean_step = b.step("clean", "Remove zig-out and .zig-cache directories");
    ...
    const example_option = b.option([]const u8, "example", "Specify which example to run");
    ...
    const run_step = b.step("run", "Run the specified example (use -Dexample=name)");

    // Create a default step to build all examples
    const all_step = b.getInstallStep();

    // Find and build all example files in the examples directory
...
fn getExampleName(filename: []const u8) []const u8 {
    var split = std.mem.splitSequence(u8, filename, ".");
    return split.first();
}
...
fn makeExample(b: *std.Build, options: BuildInfo) *std.Build.Step {
    ...
    const build_step = b.step(example_name, b.fmt("Build the {s} example", .{example_name}));
